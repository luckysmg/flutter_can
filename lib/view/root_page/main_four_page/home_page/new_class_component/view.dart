...
import 'package:neng/view/check_all_pages/all_new_class_page/page.dart';
...
Widget buildView(
    NewClassState state, Dispatch dispatch, ViewService viewService) {
  return SliverPadding(
    padding: const EdgeInsets.only(left: 16, right: 16, top: 10),
    sliver: state.newClassData == null
        ? Gap.makeSliverGap()
        : SliverStickyHeader(
            header: HomeGroupHeader(
              text: '上新课程',
              onTap: () => NavigatorUtil.push(
                  viewService.context, AllNewClassPage().buildPage(null),
                  rootNavigator: true),
            ),
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                  (context, index) => _item(state, dispatch, context, index),
                  childCount: state.newClassData.rows.length),
            ),
          ),
  );
}
...
Widget _item(
    NewClassState state, Dispatch dispatch, BuildContext context, int index) {
  var itemData = state.newClassData.rows[index];
