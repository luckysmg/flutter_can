...
import 'package:neng/view/check_all_pages/all_classical_class_page/page.dart';
...
Widget buildView(HomeState state, Dispatch dispatch, ViewService viewService) {
  return SliverPadding(
    ...
    sliver: state.goodClassData == null
        ? Gap.makeSliverGap()
        : SliverStickyHeader(
            header: HomeGroupHeader(
              text: '精品课',
              onTap: () => NavigatorUtil.push(
                  viewService.context, AllClassicalClassPage().buildPage(null),
                  rootNavigator: true),
            ),
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                  (context, index) => _item(state, dispatch, context, index),
                  childCount: state.goodClassData.rows.length <= 5
                      ? state.goodClassData.rows.length
                      : 5),
            ),
          ),
  );
...
Widget _item(
  ...
  var itemData = state.goodClassData.rows[index];
